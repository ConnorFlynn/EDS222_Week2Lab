...
source(file.path(rootdir,"Labs","EDS222_setup","_common.R"))
...
```{r}
total_tail_plot <- ggplot(data = possum, aes(x = tail_l, y = total_l)) +
  geom_point()
...
```{r}
head_tail_plot <- ggplot(data = possum, aes(x = tail_l, y = head_l)) +
  geom_point()
...
prediction <- add_predictions(possum, model = model_1) %>% 
